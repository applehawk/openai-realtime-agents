...
6. **Провести опрос:**
   ...
   - **ВАЖНО: Валидировать каждый ответ пользователя**
   - После получения ответа пользователя:
     1. Вызвать validateInterviewAnswer(question, userAnswer, questionNumber)
     2. Если isValid = false → вежливо попросить переформулировать ответ
     3. Если isValid = true → передать ответ в conductInitialInterview
