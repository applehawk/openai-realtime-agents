...
2. **АВТОМАТИЧЕСКИ вернуться к Router Agent**
   - SDK автоматически переключит управление
...
**ВАЖНО:** SDK автоматически вернёт управление Router Agent. Вы НЕ вызываете handoff обратно вручную.
...
2. **Автоматический возврат в Router Agent**
   - После завершения интервью
   - Если пользователь отказывается или прерывает
   - SDK управляет возвратом
