...
mcp-test: ## Запустить MCP Google test container
	@echo "$(GREEN)========================================$(RESET)"
	@echo "$(GREEN)Starting MCP Google test container...$(RESET)"
	@echo "$(GREEN)========================================$(RESET)"
	@if [ -z "$$MCP_REFRESH_TOKEN" ]; then \
		echo "$(YELLOW)Error: MCP_REFRESH_TOKEN environment variable is not set$(RESET)"; \
		echo "Usage: MCP_REFRESH_TOKEN=your_token GOOGLE_CLIENT_ID=your_id GOOGLE_CLIENT_SECRET=your_secret make mcp-test"; \
		exit 1; \
	fi
	@if [ -z "$$GOOGLE_CLIENT_ID" ]; then \
		echo "$(YELLOW)Error: GOOGLE_CLIENT_ID environment variable is not set$(RESET)"; \
		exit 1; \
	fi
	@if [ -z "$$GOOGLE_CLIENT_SECRET" ]; then \
		echo "$(YELLOW)Error: GOOGLE_CLIENT_SECRET environment variable is not set$(RESET)"; \
		exit 1; \
	fi
	@docker run -d \
...
mcp-stop: ## Остановить и удалить MCP Google test container
	@echo "$(YELLOW)Stopping and removing MCP Google test container...$(RESET)"
...
mcp-logs: ## Показать логи MCP Google container
	@echo "$(GREEN)========================================$(RESET)"
	@echo "$(GREEN)MCP Google container logs...$(RESET)"
	@echo "$(GREEN)Press Ctrl+C to stop viewing logs$(RESET)"
	@echo "$(GREEN)========================================$(RESET)"
	@echo ""
	@docker logs -f mcpgoogle-testuser

mcp-restart: ## Перезапустить MCP Google test container
	@echo "$(YELLOW)Restarting MCP Google test container...$(RESET)"
	@$(MAKE) mcp-stop
	@echo ""
	@$(MAKE) mcp-test
